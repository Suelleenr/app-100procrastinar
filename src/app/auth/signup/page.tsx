...
import { generateVerificationCode, createVerificationCode, sendVerificationEmail } from '@/lib/auth-helpers';
...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createVerificationCode(data.user.id, code);
      await sendVerificationEmail(email, code);

      toast.success('Conta criada! Verifique seu e-mail.');
      router.push(`/auth/verify?email=${email}&userId=${data.user.id}`);
    } catch (error: any) {
        ...
        <Card className="w-full max-w-md shadow-2xl">
          <CardHeader className="space-y-1 text-center">
            ...
            <CardDescription className="text-base">
              Comece sua jornada contra a procrastinação
            </CardDescription>
          </CardHeader>
